...
func NewPublishQueue(underlying ReservaPublisher, queueSize, maxRetries int, backoff time.Duration) *PublishQueue {
	return &PublishQueue{
...
func (q *PublishQueue) Publish(ctx context.Context, action string, reservaID string) error {
	select {
	case <-ctx.Done():
		return ctx.Err()
	default:
	...
	msg := PublishMessage{Action: action, ID: reservaID}
